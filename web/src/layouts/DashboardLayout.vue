...
const pageTitle = computed(() => {
  ...
  if (route.name === 'monitors') return 'Monitors';
  if (route.name === 'incidents') return 'Incident Logs';
  if (route.name === 'status-pages') return 'Status Pages';
  if (route.name === 'users') return 'Users';
  if (route.name?.toString().startsWith('settings')) return 'Settings';
  return 'Dashboard';
});
</script>
