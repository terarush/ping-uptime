...
<template>
  <section id="contributors" class="py-20 md:py-28 bg-muted/30">
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div v-else class="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <Card
          ...
          class="reveal group border-border/50 bg-card/60 dark:bg-card/40 text-center hover:shadow-md transition-shadow"
        ...
        >
          <a :href="c.html_url" target="_blank" rel="noopener noreferrer" class="block p-6">
            ...
            <p class="text-xs text-muted-foreground mt-1">{{ c.contributions }} commit{{ c.contributions !== 1 ? 's' : '' }}</p>
            <span class="mt-3 inline-flex items-center gap-1 text-xs font-medium text-primary/80 hover:text-primary transition-colors">
              View profile &rarr;
            </span>
          </a>
        </Card>
